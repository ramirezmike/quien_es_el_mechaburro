...
use crate::{ game_camera::PanOrbitCamera, ui::text_size, assets };
...
pub trait FollowTextCommandsExt {
    fn spawn_follow_text<T: Component>(&mut self, 
...
impl<T: Component> Command for SpawnFollowText<T> {
    fn apply(self, world: &mut World) {
        ...
        world 
            ...
            .insert(FollowText { following: self.follow_entity, offset: 0.0, });
    }
}
        ...
        if let Ok(mesh_position) = mesh_query.get(follow_text.following) {
            ...
            for (camera, camera_transform) in camera_query.iter() {
                ...
                match camera.world_to_viewport(camera_transform, translation) {
                    Some(coords) => {
                        style.left = Val::Px(coords.x).try_sub(Val::Px(text_layout_info.size.x / 2.0)).unwrap();
                        style.top = Val::Px(coords.y).try_sub(Val::Px(text_layout_info.size.y / 2.0)).unwrap();
                    }
                    None => {
